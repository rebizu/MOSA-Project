        ...
        public KeyEvent ConvertScanCode(byte scancode)
        {
            ...
            if (keyState == KeyState.Normal)
            {
                ...
                switch (scancode)
                {
                    ...
                    case 205: key.Character = (char)0; break;
                    case 206: key.KeyType = KeyType.RightArrow; break;
                    case 207: key.Character = '+'; break;
                    case 208: key.KeyType = KeyType.End; break;
                    case 209: key.KeyType = KeyType.DownArrow; break;
                    case 210: key.KeyType = KeyType.PageDown; break;
                    case 211: key.KeyType = KeyType.Insert; break;
                    case 214: key.KeyType = KeyType.Delete; break;
                    case 215: key.Character = '>'; break;
                    case 216: key.KeyType = KeyType.F11; break;
                    case 217: key.KeyType = KeyType.F12; break;
                }
                keyState = KeyState.Normal;
                return key;
            }
            else if ((keyState == KeyState.Escaped) || (keyState == KeyState.EscapedBreak))
            {
                if(scancode == 0xE0)
                {
                    key.KeyType = KeyType.RegularKey;
                    if ((scancode & 0x80) != 0 || (keyState == KeyState.EscapedBreak))
                        key.KeyPress = KeyEvent.Press.Make;
                    else
                        key.KeyPress = KeyEvent.Press.Make;
                    ...
                    if(scancode == 0xF0)
                    {
                        keyState = KeyState.EscapedBreak;
                        return key;
                    }
                ...
                else if(keyState == KeyState.Escaped2)
                {
                    keyState = KeyState.Normal;
                    return key;
                }
